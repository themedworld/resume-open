...
const Sidebar = ({
  ...
  const [receivers, setSenders] = useState<any[]>([]); // State to hold the senders data
  const [selectedSenderId, setSelectedSenderId] = useState<number | null>(null); // State to hold selected sender ID
  ...
  const fetchContact = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/v1/message/contacts/${userId}`, {
        method: "GET",
        headers: {
          ...
          {receivers?.map((receiver) => (
            <li
              className={classNames({
                ...
                "bg-green-900": selectedSenderId === receiver.id,
              })}
              key={receiver.id}
              onClick={() => handleSenderClick(receiver.id)
      ...
      <div className="border-t border-t-indigo-800 p-4">
        <div className="flex gap-4 items-center">
          <div className="flex flex-col ">
            ...
            <Link href="/Recruteur" className="text-indigo-200 text-sm">
            <ArrowLeftIcon className="w-6 h-6 text-green-500" />
            </Link>
          </div>
        </div>
      </div>
